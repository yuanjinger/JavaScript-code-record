...
(function ($) {
	$.fn.fxdHdrCol = function (o) {
		var cfg = {
			...
			console.log("f"+cfg.fixedCols);//******************
		$.extend(cfg, o);
			console.log("s"+cfg.fixedCols);//******************
		return this.each (function () {
			...
			/* set width and textAlign from colModal （设置宽度和对齐方式）*/
			var ct = 0;
	...
	//**********************************************
			if (cfg.fixedCols > 0) {
				...
				//calculate the actual column's count (support for colspan)	（计算实际的列数（支持合并单元格））				
				var r1c1ColSpan = 0;		
				for (var i = 0; i < cfg.fixedCols; i++ ) {
					r1c1ColSpan += this.rows[0].cells[i].colSpan;			
				}					
				
				//prepare rows/cols for fixed row col section（准备行/列固定行列部分
				var tdct = 0;
